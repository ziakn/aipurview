        ...
        <Section className="bg-background">
            <div className="flex flex-col lg:flex-row gap-16">
                <div className="lg:w-1/3">
                    ...
                    <h2 className="text-3xl md:text-4xl font-black mb-6 text-foreground">Useful context before we talk</h2>
                    ...
                    <div className="p-6 glass-card">
                        ...
                        <a href="/contact" className="text-sm font-bold text-brand-orange hover:underline">Book a consultation</a>
                    </div>
                </div>
                ...
                <div className="lg:w-2/3 space-y-4">
                    {faqs.map((faq, index) => (
                        <div
                            key={index}
                            className={`glass-card border-card-border overflow-hidden transition-colors duration-200 ${openIndex === index ? "bg-dark-charcoal" : "hover:bg-dark-charcoal/70"}`}
                        >
                            <button
                                onClick={() => setOpenIndex(openIndex === index ? null : index)}
                                className="w-full p-6 text-left flex items-center justify-between"
                            >
                                <span className="font-bold text-foreground/85 transition-colors">
                                    {faq.question}
                                </span>
                                <div className={`shrink-0 ml-4 p-2 rounded-md ${openIndex === index ? "bg-brand-orange text-background" : "bg-foreground/5 text-foreground/50"}`}>
                            ...
                            >
                                <div className="px-6 pb-6 pt-0 text-foreground/50 text-sm leading-relaxed border-t border-card-border">
                                    <div className="pt-6">
                                        {faq.answer}
                                    </div>
                                </div>
